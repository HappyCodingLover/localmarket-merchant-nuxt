    ...
    <div>
        ...
        <div v-if="listShow" class="zone-lists mt-10">
            ...
            >
                ...
                <v-col
                    md="5"
                    class="grey-text pt-0 pb-0 pl-10 pr-10 center zone-timeframe-list"
                >
                    <span
                        class="item"
                        v-for="(item, i) in zone.delivery_timeframes"
                        :key="i"
                    >
                        {{ item.start + " - " + item.end }}
        ...
        <div v-if="detailShow" class="mt-4">
            ...
            <v-row class="mt-4">
                ...
                <v-col md="6" sm="6" xs="10">
                    <span
                        >Сегодня / Завтра / Послезавтра</span
                    >
                    <v-autocomplete
                        hide-details
                        outlined
                        ...
                        color="#414042"
                        :items="detailDays"
                        v-model="detailDeliveryZoneDays"
                        @change="changeDetailValue()"
                        multiple
                        return-object
                    >
            ...
            <div class="mt-10 flex-layout">
                ...
                <span class="ml-5 mt-1 grey-text"
                    >После запроса изменения, данные<br />будут отправлены на
                    модерацию.</span
                >
            </div>
            ...
            <v-row class="mt-4">
                ...
                <v-col md="6" sm="6" xs="10">
                    ...
                    <v-autocomplete
                        ...
                        color="#414042"
                        :items="newDays"
                        v-model="newSelectedDays"
                        multiple
                        return-object
                        :search-input.sync="searchNewDay"
                    >
                    </v-autocomplete>
                </v-col>
                <v-col md="6" sm="6" xs="10">
        ...
        <v-dialog v-model="confirmDialog" width="500">
            <v-card>
                ...
                <v-divider></v-divider>
                <p class="pa-10">
                    Вы уверены, что хотите удалить эту зону доставки?
                </p>
                <v-divider></v-divider>
                <v-card-actions>
                    <v-spacer></v-spacer>
...
export default {
    ...
    mounted() {
        console.log('__store', this.$store.state);
        this.delivery_zones = this.$store.state.delivery_zones
            ? this.$store.state.delivery_zones
            : [];
        ...
        if (this.$store.state.days) {
            this.days = [];
            let tmp_newDays = [];
            this.newDays = [];
            this.$store.state.days.map((day) => {
                this.days.push(day.text);
                this.newDays.push({ text: day.text, value: day.value });
                tmp_newDays.push(day.text);
            });
        }
    ...
    watch: {
        "$store.state.delivery_zones": function () {
            ...
            if (this.$store.state.districts) {
                ...
                this.delivery_zones.map((zone) => {
                    ...
                    this.districtRegions.map((region) => {
                        if (region === zone.district) {
                            this.newDistricts = this.newDistricts.filter(
                                (item) => {
                                    if (item.region === region) {
                                        return false;
                                    } else {
            ...
            this.newSelectedDistricts.map((district) => {
                this.districtRegions.map((region) => {
                    if (district.text === region) {
                        this.newDistricts.map((new_district) => {
                            if (
                                new_district.region === region &&
                                district.value !== new_district.value
                            ) {
        ...
        newSelectedDays: function () {
            this.newDays.map((newDay) => {
                if (newDay.value) {
                    newDay.disabled = false;
                }
            });
            this.newSelectedDays.map((day) => {
                day.disabled = false;
            });
            this.searchNewDay = null;
        },
        // detailDeliveryZoneDays: function () {
        //     this.detailDays.map((day) => {
        //         if (day.value) {
        //             day.disabled = false;
        //         }
        //     });
        //     this.detailDeliveryZoneDays.map((day) => {
        //         day.disabled = false;
        //     });
        // },
    },
    methods: {
        ...
        viewDeliveryZoneByIndex(_index) {
            ...
            this.detailDeliveryZoneDistrict = this.detailDistricts[
                this.detailDistricts.indexOf(this.detailDeliveryZone.district)
            ];
            try {
                console.log(this.detailDeliveryZone.days)
                JSON.parse(this.detailDeliveryZone.days).map((day) => {
                    let dayText;
                    if (day === 0) {
                        dayText = "Сегодня";
                    } else if (day === 1) {
                    ...
                    this.detailDeliveryZoneDays.push({text: dayText, value: day});
                });
            } catch(e){
                console.log(e);
            }
        ...
        addDeliveryTime() {
            ...
            this.delivery_times.push({
                text: "00:00 - 00:00",
                value: this.delivery_times.length,
            });
        ...
        addNewDeliveryConfirm() {
            if (
                ...
                this.newSelectedDistricts.length === 0
            ) {
            ...
            if (!this.newSelectedDays || this.newSelectedDays.length === 0) {
                this.$store.commit("setAlert", {
                    show: true,
                    text: "Пожалуйста, выберите день доставки",
                });
                return;
            }
            if (
                !this.newDeliveryTimeFrames ||
                this.newDeliveryTimeFrames.length === 0
            ) {
                this.$store.commit("setAlert", {
                    show: true,
            ...
            if (
                !this.newDelivery_price ||
                this.newDelivery_price.trim() === ""
            ) {
                this.$store.commit("setAlert", {
                    show: true,
            ...
            if (
                !this.newMin_order_price ||
                this.newMin_order_price.trim() === ""
            ) {
                this.$store.commit("setAlert", {
                    show: true,
            ...
            if (
                !this.newFree_delivery_from ||
                this.newFree_delivery_from.trim() === ""
            ) {
                this.$store.commit("setAlert", {
                    show: true,
                    text:
                        "Пожалуйста, введите сумму, с которой начинается бесплатная доставка",
                });
                return;
            }
        ...
        addNewDeliveryZone() {
            ...
            let days_ids = [];
            this.newSelectedDays.map((day) => {
                if (days_ids.indexOf(day.value) === -1 && !day.disabled) {
                    days_ids.push(day.value);
                }
            });
            const data = {
                days_ids,
        ...
        reviseDeliveryZoneSendRequest() {
            ...
            if (parseFloat(this.detailDeliveryZoneDeliveryPrice) != this.detailDeliveryZoneDeliveryPrice ) 
            {
                this.$store.commit("setAlert", {
                    show: true,
                    text: "Пожалуйста, введите стоимость доставки",
            ...
            if (
                !this.detailDeliveryZoneMinOrderPrice &&
                this.detailDeliveryZoneMinOrderPrice.trim() === ""
            ) {
                this.$store.commit("setAlert", {
                    show: true,
                    text: "Пожалуйста, введите минимальную сумму заказа",
            ...
            if (
                !this.detailDeliveryZoneFreeDeliveryFrom &&
                this.detailDeliveryZoneFreeDeliveryFrom.trim() === ""
            ) {
                this.$store.commit("setAlert", {
                    show: true,
                    text:
            ...
            console.log("__days", days_ids);
            const data = {
                days_ids,
                district_id,
                delivery_timeframes: this.detailDeliveryZone
                    .delivery_timeframes,
            ...
            this.$store
                .dispatch("reviseDeliveryZone", {
                    data,
                    id: this.detailDeliveryZone.id,
                })
                .then(() => {
                    this.detailDeliveryZoneDistrict = null;
                    this.detailDeliveryZoneDays = [];
